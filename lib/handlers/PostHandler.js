...
class PostHandler extends BaseHandler {
     ...
     * @param  {object} options list of options (filename, onSuccess, onError)
     * @return {Promise}        resolves to {ServerResponse}
     */
    send(req, res, options) {
        const length = parseInt(req.headers['upload-length'], 10);
        const deferred_length = parseInt(req.headers['upload-defer-length'], 10);
        ...
        const file = new File(length || deferred_length);
        ...
        if (req.headers['upload-metadata']) {
            metadata = this._normalizeMetadata(req.headers['upload-metadata']);
        ...
        fileOperationPromises.push(this.store.saveMetadata(file.id, metadata));
