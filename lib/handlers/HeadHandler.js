    ...
    send(req, res, options) {
        ...
        if (!match) {
            if (options && options.error && typeof options.error === 'function') {
                ...
                err.status = 412;
                return options.error(err, 412);
            }
            return super.send(res, 404);
        }

        const file_name = match[1];
        return this.store.getMetadata(file_name)
            .then((metadata) => {
                return Promise.all([this.store.getOffset(file_name), metadata]);
            }, (err) => {
                console.warn('[HeadHandler] send: No Metadata found.');
                return Promise.all([this.store.getOffset(file_name), null]);
            })
            .then((data) => {
                const stats = data[0];
