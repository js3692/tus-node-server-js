...
class FileStore extends DataStore {
    constructor(options) {
        ...
        this.directory = `${__dirname}/${options.directory}` || options.path.replace(/^\//, '');
    ...
    /**
     * Called in POST requests. This method saves the metadata
     * as a JSON.
     *
    ...
    getMetadata(file_name) {
        return new Promise((resolve, reject) => {
            fs.readFile(this.directory, (err, metadata) => {
                if (err) {
                    return reject(err);
                }
